...
const char* googleSheetURL = "https://script.google.com/macros/s/AKfycbxKCQ3LoNUsp44V3ymGklTtC9lf4DHxgR0TOZYo9nIOio777Jyavs1hW66TwTcCd4hg/exec";

Servo dustServo;  // Khai báo đối tượng servo
int servoPin = 13;  // Cổng servo kết nối
...
void setup(){
  Serial.begin(115200);
  // Khởi tạo servo ở chân 13
  dustServo.attach(13);  // Cổng 13 cho servo
  dustServo.write(0);    // Đặt servo về vị trí ban đầu (0 độ)
...
// gửi dữ liệu
void sendDataToGoogleSheet(float temp, int humi, int dust) {
...
void loop() {
  delay(10);  // Giảm tải cho vi điều khiển
}
...
void TaskDHT11(void *pvParameters) { 
    ...
    while(1) {
      ...
      if (isnan(humi) || isnan(temp) ) {
          ...
          sendDataToGoogleSheet(tempValue, humiValue, dustValue); // Gửi dữ liệu
      }
      delay(3000);
    }
}
...
int servoState = 0;  // Biến cờ để theo dõi trạng thái của servo (0 = góc 0 độ, 1 = góc 35 độ)
int action = 0;
...
void TaskDustSensor(void *pvParameters) {
    while(1) {
        ...
        if (action == 0){
          // Điều khiển servo khi giá trị bụi vượt qua ngưỡng
          if (dustValue > EdustThreshold) {
            dustServo.write(35);  // Servo quay 35 độ
            servoState = 1;  // Cập nhật trạng thái của servo
            Serial.println("Servo quay 35 do");
          } else {
            dustServo.write(0);   // Servo quay về 0 độ
            servoState = 0;  // Cập nhật trạng thái của servo
...
void getDataFromClient(AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
  ...
  if(myObject.hasOwnProperty("dustThreshold"))  // Nhận giá trị ngưỡng bụi hiện tại từ web
    EdustThreshold = (int) myObject["dustThreshold"];
  writeEEPROM();
...
//int servoState = 0;  // Biến cờ để theo dõi trạng thái của servo (0 = góc 0 độ, 1 = góc 90 độ)
void button_press_short_callback(uint8_t button_id) {
    switch(button_id) {
      case BUTTON1_ID :  
        buzzerBeep(1);
        Serial.println("btSET press short");
        // Kiểm tra nếu action đang là 1, đặt lại thành 0
            if (action == 1) {
                action = 0;  // Đặt action về 0
                Serial.println("Action reset to 0. Servo is active now.");
            } else {
                action = 1;  // Nếu action là 0, đặt lại thành 1 (ngừng điều khiển servo)
                Serial.println("Action set to 1. Servo is deactivated.");
            }
            break;
        break;
      case BUTTON2_ID :
        ...
        Serial.println("Servo State: " + String(servoState));  // In trạng thái servo
        if (action == 1){
          if (servoState == 0) {
              dustServo.write(35);  
              servoState = 1;
